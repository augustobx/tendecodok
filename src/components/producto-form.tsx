  ...
  const form = useForm<any>({
    ...
    defaultValues: initialData
      ? {
        ...
        litros_por_unidad: initialData.litros_por_unidad ?? 0,
        ...
        <div className="space-y-8">
          <Card className="shadow-sm border-muted/50">
            ...
            <CardContent className="space-y-6">
              ...
              {/* LITROS POR UNIDAD (NUEVO) */}
              <div className="space-y-3">
                <Label>Volumen en Litros <span className="text-slate-400 text-xs font-normal">(Para métricas)</span></Label>
                <div className="relative">
                  <Input type="number" step="0.01" {...register("litros_por_unidad")} className="bg-muted/30 pr-10" placeholder="Ej: 20" />
                  <span className="absolute right-3 top-2 text-muted-foreground text-sm">Lts.</span>
                </div>
                <p className="text-[10px] text-muted-foreground leading-tight">
                  Indica cuántos litros trae esta unidad (ej: Balde = 20, Lata = 4). Dejar en 0 para accesorios (pinceles, cintas).
                </p>
              </div>
