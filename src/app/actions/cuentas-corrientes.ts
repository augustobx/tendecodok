...
// ==========================================
// 5. HISTORIAL DE NOTAS DE CRÉDITO
// ==========================================
export async function getHistorialNotasCredito() {
    try {
        const notas = await prisma.movimientoCuentaCorriente.findMany({
            where: { tipo: 'ABONO', notas: { contains: 'Nota de Crédito' } },
            include: { cliente: true, venta: true },
            orderBy: { fecha: 'desc' }
        });
        return { success: true, data: notas };
    } catch (error: any) {
        return { success: false, error: "Error al cargar notas de crédito." };
    }
}