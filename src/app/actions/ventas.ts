    ...
    try {
        const resultado = await prisma.$transaction(async (tx) => {
                ...
                data: {
                    ...
                    pagos: {
                        create: pagos.map((p: any) => ({
                            ...
                            monto: p.monto,
                            detalle_cheque: p.detalle_cheque || null
                        }))
                    }
                }
            });
            ...
            for (const pago of pagos) {
                if (pago.metodo_pago === 'CONTADO' || pago.metodo_pago === 'TARJETA' || pago.metodo_pago === 'TRANSFERENCIA' || pago.metodo_pago === 'CHEQUE') {
                    await tx.movimientoCaja.create({
                        data: {
                            ...
                            detalle_cheque: pago.detalle_cheque || null
                        }
                    });
