            ...
            {tabActiva === "VOLUMEN" && datosVolumen && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Card className="bg-indigo-600 border-indigo-700 shadow-sm">
                        ...
                        <Card className="bg-slate-800 border-slate-900 shadow-sm">
                            <CardContent className="p-6 flex items-center gap-4">
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        {Object.entries(datosVolumen.desglose).length === 0 ? (
                            <p className="text-muted-foreground text-center py-10 bg-slate-50 dark:bg-zinc-800/50 rounded-xl border border-slate-200 border-dashed">No hay ventas registradas en este rango de fechas.</p>
                        ) : (
                            Object.entries(datosVolumen.desglose).map(([proveedor, datosProv]: [string, any]) => (
                                <Card key={proveedor} className="shadow-sm border-slate-200 overflow-hidden">
                                    <CardHeader className="bg-slate-50 border-b border-slate-100 p-4">
                                        <div className="flex justify-between items-center">
                                            <CardTitle className="text-base text-slate-800 uppercase tracking-tight font-black flex items-center gap-2"><Package className="h-4 w-4 text-slate-400" /> {proveedor}</CardTitle>
                                            <div className="text-right">
                                                <Badge className="bg-indigo-100 text-indigo-800 hover:bg-indigo-100 font-black mr-2 text-xs">
                                                    {datosProv.litros} Lts.
                                                </Badge>
                                                <Badge className="bg-slate-200 text-slate-700 hover:bg-slate-200 font-bold text-xs">
                                        ...
                                        <table className="w-full text-sm text-left">
                                            <thead className="bg-white text-slate-400 uppercase text-[10px] border-b border-slate-100">
                                                <tr>
                                                    ...
                                                    <th className="px-6 py-3 text-right font-semibold text-indigo-600">Litros Vendidos</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-slate-100 bg-white">
                                                {Object.entries(datosProv.marcas).map(([marca, metricas]: [string, any]) => (
                                                    <tr key={marca} className="hover:bg-slate-50/50 transition-colors">
                                                        <td className="px-6 py-3 font-medium text-slate-700">{marca}</td>
                                                        <td className="px-6 py-3 text-right font-semibold text-slate-500">{metricas.unidades}</td>
                                                        <td className="px-6 py-3 text-right font-black text-indigo-600">{metricas.litros} Lts.</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
