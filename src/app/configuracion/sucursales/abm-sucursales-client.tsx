            ...
            <div className="space-y-6">
                {sucursales.map(sucursal => (
                    <Card key={sucursal.id} className="border-slate-200 dark:border-zinc-800 shadow-sm overflow-hidden bg-white dark:bg-zinc-900">
                        <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-5 bg-slate-50/50 dark:bg-zinc-900/50 border-b border-slate-100 dark:border-zinc-800 gap-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Badge variant="outline" className={`text-[10px] font-bold tracking-widest uppercase ${sucursal.estado ? 'border-emerald-200 bg-emerald-50 text-emerald-600' : 'border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800 text-slate-500 dark:text-slate-400'}`}>
                            ...
                            <Button size="sm" variant="outline" onClick={() => abrirEditarSucursal(sucursal)} className="bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-800">
                                <Edit className="h-3.5 w-3.5 mr-1.5 text-slate-500 dark:text-slate-400" /> Editar
                            </Button>
                        </div>
                        ...
                        <CardContent className="p-0">
                            <div className="px-5 py-3 bg-white dark:bg-zinc-900 flex items-center justify-between border-b border-slate-100 dark:border-zinc-800">
                                <h4 className="text-xs uppercase font-bold text-slate-500 dark:text-slate-400 flex items-center gap-2 tracking-widest">
                            ...
                            <div className="divide-y divide-slate-100 dark:divide-zinc-800">
                                {sucursal.depositos.map((depo: any) => (
                                    <div key={depo.id} className="flex items-center justify-between px-5 py-3 hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors">
                ...
                {sucursales.length === 0 && (
                    <div className="text-center py-16 border-2 border-dashed border-slate-200 dark:border-zinc-800 rounded-2xl">
                        ...
                        <h3 className="text-lg font-bold text-slate-700 dark:text-slate-200">No hay sucursales cargadas</h3>
                        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Crea tu primera sucursal física para comenzar a facturar.</p>
            ...
            {modalSuc && (
                <div className="fixed inset-0 z-50 flex justify-center items-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in">
                    <Card className="w-full max-w-md shadow-2xl border-none">
                        <CardHeader className="bg-slate-50 dark:bg-zinc-800 border-b border-slate-100 flex flex-row items-center justify-between p-5">
                        ...
                        <CardContent className="p-5 space-y-4">
                            <div className="space-y-1.5">
                                <Label className="text-xs uppercase font-bold text-slate-500 dark:text-slate-400">Nombre de Sucursal <span className="text-red-500">*</span></Label>
                                <Input value={formSuc.nombre} onChange={e => setFormSuc({ ...formSuc, nombre: e.target.value })} placeholder="Ej: Sucursal Centro" autoFocus />
                            </div>
                            <div className="space-y-1.5">
                                <Label className="text-xs uppercase font-bold text-slate-500 dark:text-slate-400">Dirección</Label>
                                <Input value={formSuc.direccion} onChange={e => setFormSuc({ ...formSuc, direccion: e.target.value })} placeholder="Ej: Av. San Martín 1234" />
                            </div>
                            <div className="space-y-1.5">
                                <Label className="text-xs uppercase font-bold text-slate-500 dark:text-slate-400">Teléfono</Label>
                                <Input value={formSuc.telefono} onChange={e => setFormSuc({ ...formSuc, telefono: e.target.value })} placeholder="Ej: 341 555-4444" />
                            </div>
                            {sucEditando && (
                                <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-zinc-800 rounded-xl border border-slate-200 dark:border-zinc-800 mt-2">
                                    <Label className="text-sm font-bold text-slate-700 dark:text-slate-200 cursor-pointer" htmlFor="estado-suc">Sucursal Operativa (Activa)</Label>
            ...
            {modalDepo && (
                <div className="fixed inset-0 z-50 flex justify-center items-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in">
                    <Card className="w-full max-w-sm shadow-2xl border-none">
                        ...
                        <CardContent className="p-5 space-y-4">
                            <div className="space-y-1.5">
                                <Label className="text-xs uppercase font-bold text-slate-500 dark:text-slate-400">Nombre del Depósito <span className="text-red-500">*</span></Label>
                            ...
                            {depoEditando && (
                                <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-zinc-800 rounded-xl border border-slate-200 dark:border-zinc-800 mt-2">
                                    <Label className="text-sm font-bold text-slate-700 dark:text-slate-200 cursor-pointer" htmlFor="estado-depo">Depósito Activo</Label>
