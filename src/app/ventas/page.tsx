...
function PosTerminal({ tabId, allOtherCarts, updateCartInfo }: any) {
    ...
    const [fechaEmision, setFechaEmision] = useState(new Date().toLocaleDateString('en-CA', { year: 'numeric', month: '2-digit', day: '2-digit' }).replace(/\//g, '-'));
    ...
    // === PAGOS MÚLTIPLES ===
    const [pagos, setPagos] = useState<{ metodo_pago: string; monto: string; cuotas?: string; recargo_porcentaje?: string, detalle_cheque?: string }[]>([
        { metodo_pago: "CONTADO", monto: "" }
    ]);
    const [presupuestoOrigenId, setPresupuestoOrigenId] = useState<number | null>(null);
    ...
    useEffect(() => {
        const init = async () => {
            ...
            const pId = urlParams.get("presupuesto_id");
            if (pId && tabId === 1) { // Solo cargarlo en la primera pestaña
                const presRes = await getPresupuestoById(Number(pId));
                if (presRes.success && presRes.data) {
                    const pres = presRes.data;
                    setPresupuestoOrigenId(pres.id);
                    setClienteSeleccionado(pres.cliente);
                    setListaPrecioSeleccionada(String(pres.listaPrecioId));
                    setDescuentoGlobal(pres.descuento_global > 0 ? (pres.descuento_global * 100 / pres.subtotal) : 0);
                    
                    const newCarrito = pres.detalles.map((d: any) => ({
                        ...
                        subtotal: d.subtotal,
                        stock_actual: d.producto.stocks?.[0]?.cantidad || 0,
                        tipo_medicion: d.producto.tipo_medicion || "UNIDAD",
                        _rawProducto: d.producto
                    }));
    ...
    const handleProcesarVenta = () => {
        ...
        const pagosValidos = pagos.map(p => ({ 
            ...
            detalle_cheque: p.detalle_cheque || undefined
        })).filter(p => p.monto > 0);
        if (pagosValidos.length === 0) {
            ...
            pagosValidos.push({ 
                ...
                detalle_cheque: pagos[0].detalle_cheque
            });
        }
